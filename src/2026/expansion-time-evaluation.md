...
We will implement an experimental attribute (e.g., `#[rustc_expansion_time]`) that restricts a function's body to only see trait implementations from _upstream_ crates. By opting out of seeing the current crate's implementations, these functions can be safely compiled and evaluated before the rest of the current crate has finished expanding.
...
No. This work is strictly about the compiler architecture needed to make such a feature _possible_ to implement in the future.
