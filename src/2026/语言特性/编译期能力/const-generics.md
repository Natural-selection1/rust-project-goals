...
While we're currently only looking to stabilize uses of Structs/Enums in Const Generics, that isn't everything that _should_ be supported as the types of const generics.
...
`feature(min_generic_const_args)` currently exists as a prototype of a potential _minimal_ stabilizeable improvement. We intend to make more progress on the implementation for this feature so that it is more ready for stabilization.
...
We intend to explore support for strings _after_ stabilizing structs/enums in Const Generics as they are much more complicated than basic support for structs/enums.
