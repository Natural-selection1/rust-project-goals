...
Related but distinct. Custom receivers (RFC 3519) allow methods with `self: MyPtr<Self>`. The "Beyond the `&`" work goes further: making those custom pointer types _feel_ like native references through automatic reborrowing, field access, and borrow checker integration.
